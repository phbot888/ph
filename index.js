...
limitt = 50
ban = []
userpremium = ["5568999638546@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola @${num.split('@')[0]}\nBem vindo (a) E NÃO SEJA GH0ST☕ *${mdata.subject}*`
				...
				teks = `Tchau e nao volte arrombado @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ 𝗰𝗮𝗹𝗺𝗮 𝗮í 𝗺𝗮𝗻𝗼 come o cu do baiano ou 𝘁𝗼𝗺𝗲 𝘂𝗺 𝗰𝗮𝗳é☕𝗲 𝘂𝗺 𝗯𝗶𝘀𝗰𝗼𝗶𝘁𝗼 🥯 𝗽𝗼𝗿 𝗲𝗻𝗾𝘂𝗮𝗻𝘁𝗼...*',
				success: '*ꜱᴜceꜱꜱo...*',
				error: {
				...
				only: {
					...
					ownerB: '*𝘀o 𝗼 𝗱𝗼𝗻𝗼 𝗱𝗼 𝗯𝗼𝘁 PH GOSTOZAUM 𝗽𝗼𝗱𝗲 𝘂𝘀𝗮𝗿 𝗲𝘀𝘀𝗲 𝗰𝗼𝗺𝗮𝗻𝗱𝗼 !* ',
					...
					admin: '*sᴏᴍᴇɴᴛᴇ ᴏ ᴀᴅᴍ ᴘᴏᴅᴇ ᴜsᴀʀ ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ ENTÃO VIRE UMHAHA !*',
